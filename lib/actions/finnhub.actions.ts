...
export async function getWatchlistData(symbols: string[]): Promise<StockWithData[]> {
    ...
    try {
            ...
            symbols.map(async (symbol) => {
                try {
                    ...
                    const [quote, profile, metrics] = await Promise.all([
                        ...
                        fetchJSON<FinancialsData>(metricUrl, 3600),
                    ]);

                    const peRatio = metrics?.metric?.peExclExtraTTM?.toFixed(1) || metrics?.metric?.peInclExtraTTM?.toFixed(1) || 'N/A';
                    ...
                    return {
                        ...
                        marketCap: formatMarketCapValue((profile?.marketCapitalization || 0) * 1e6), 
                        peRatio: peRatio,
                    } as StockWithData;
                } catch (e) {
        ...
        return enrichedData.filter((d): d is StockWithData => d !== null);
    } catch (err) {
